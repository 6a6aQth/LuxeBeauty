  ...
  return (
    <div className="bg-[#faf7f8]">
      <div className="max-w-5xl mx-auto px-4 py-10 md:py-14">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="mt-2 font-serif text-4xl text-gray-900 md:text-5xl">
              {payload ? `Hello, ${firstName}` : "Your account"}
            </h1>
            {payload && (
              <p className="mt-2 text-sm text-gray-500">Visits for {payload.phone}</p>
            )}
          </div>
        </div>
        ...
        {!payload && !error && (
          <p className="mt-10 text-sm text-gray-500">Loading your visits…</p>
        )}
              ...
              <section>
                ...
                {payload.upcoming.length === 0 ? (
                  <div className="mt-4 rounded-2xl border border-dashed border-pink-200 bg-white px-6 py-10 text-center">
                    ...
                    <p className="mt-1 text-sm text-gray-500">Your next appointment will show up here after payment.</p>
                  </div>
                ) : (
                    ...
                    {payload.upcoming.map((visit) => (
                      <li key={visit.id} className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                        ...
                        <div className="mt-4 flex flex-wrap gap-3">
                          <Button className="rounded-none" onClick={() => setOpenTicket(visit)}>View ticket</Button>
      ...
      <Dialog open={Boolean(openTicket)} onOpenChange={(open) => !open && setOpenTicket(null)}>
        <DialogContent className="max-w-md border-0 bg-gray-200 p-6 sm:max-w-md">
          <DialogHeader>
