...
One version of that bug still happens if the browser waits on `chargeMobileMoney()` before it starts polling. On a phone — iOS Safari on a weak connection is the usual case — that request dies with `TypeError: Load failed` (or `Failed to fetch`) after PayChangu has already sent the PIN prompt and the customer has already approved it. The page then shows a payment error, never polls, never sends the receipt, and the money is already gone. Save the charge id and show "check your phone" before the initialize call, and start verify on that id even when the initialize request never returns. A dropped initialize call is not a failed payment. Only `verifyDirectCharge()` returning a failed status is.
